...
from companion_core.supervisor import Supervisor, Worker, worker_loop
...
class TestSupervisor(unittest.TestCase):
    ...
    def test_start_spawns_one_thread_per_worker(self):
        ...
        ticks = []
        sup = Supervisor(
            [Worker("a", lambda: ticks.append("a"), 0.0)], spawn=spawn, sleeper=lambda s: None, max_ticks=2
        )
        sup.start()
    ...
    def test_stop_is_idempotent_and_sets_not_running(self):
        spawn, made = self._spawn()
        sup = Supervisor([Worker("a", lambda: None, 0.0)], spawn=spawn, sleeper=lambda s: None, max_ticks=1)
        sup.start()
        sup.stop()
        sup.stop()
