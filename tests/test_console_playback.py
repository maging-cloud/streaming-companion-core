...
class TestPlayer(unittest.TestCase):
    ...
    def test_writes_wav_then_calls_popen_on_mac(self):
        calls = []
        player = make_player(play_path=self.path, platform="darwin", popen=lambda args: calls.append(args))
        player(b"RIFFdata")
    ...
    def test_linux_uses_aplay(self):
        calls = []
        player = make_player(play_path=self.path, platform="linux", popen=lambda args: calls.append(args))
        player(b"x")
    ...
    def test_windows_uses_winsound(self):
        ...
        player = make_player(play_path=self.path, platform="win32", winsound_mod=_WS())
        player(b"x")
