...
provider 契約 (duck typing):
    label: str                                   # 表示名 (任意)
    synth(config)  -> callable(text)->wav | None  # TTS 合成 (任意)
    player(config) -> callable(wav) | None        # 音声再生 (任意)
...
def build_service(provider, config, *, console_state=None, service_cls=None,
    ...
    synth = provider.synth(config) if hasattr(provider, "synth") else None
    player = provider.player(config) if hasattr(provider, "player") else None
