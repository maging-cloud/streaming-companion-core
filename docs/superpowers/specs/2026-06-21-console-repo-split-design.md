...
## panel 契約(console 側、3段)

plugin が console にパネルを出す手段。**UI ゼロの plugin は schema か ui_file を使う**:

| パス | plugin が提供 | 複雑さ | plugin の PySide6 依存 |
|---|---|---|---|
| `schema`(JSON Schema, dict) | dict | 単純フォーム | なし(frontend 非依存) |
| **`ui_file`(Qt Designer .ui)** | `.ui` XML ファイルパス | 任意に複雑なレイアウト | なし(.ui は data ファイル) |
| `build_widget(cfg)->QWidget` | QWidget | 完全自由(動的/カスタム描画) | あり(UI ゼロ plugin では不可) |

判定(console の `registry`/`window`、duck typing): `build_widget` があれば命令的、無く `ui_file`
があれば .ui ロード、無く `schema` があれば JSON Schema。

### ui_file パスの実装(console 側)

- `schema_ui.build_form_from_ui(ui_path)`: `QUiLoader().load(ui_path)` で QWidget 化し、
  **objectName を config キーとみなして**入力 widget(QLineEdit/QSpinBox/QDoubleSpinBox/
  QCheckBox/QComboBox)を型ごとに汎用 get/set する。戻りは `(widget, get_values, set_values)`
  で `build_form` と同じ triple。
- plugin は `.ui` を同梱し objectName を config キーに合わせるだけ(Qt Designer で作成)。**PySide6 import なし。**
- `schema`(frontend 非依存)と `ui_file`(Qt 専用・複雑)を用途で使い分ける。

## companion_bpb: UI を持たずに console パネルを定義する

**bpb は config が単純(path/数値/bool)なので宣言的 `schema` パスを使う**(`.ui` も可だが不要):
...
3. core repo の `companion_settings` を console repo へ移設し `companion_console` に rename。
   `schema_ui` に **boolean→QCheckBox** と **`build_form_from_ui`(QUiLoader + objectName get/set)** を追加。
   `registry`/`window` の panel 判定を 3 段(build_widget / ui_file / schema)に拡張。
   console は `streaming-companion-core` を依存(git pin)。
